...
const Login = () => {
  ...
  return (
    <div className=" h-screen flex   justify-center items-center">
      <form
        className="h-auto w-96 shadow hover:shadow-lg   flex   p-3 gap-3 flex-col align-items-center"
        onSubmit={handleSubmit}
      >
        <legend>{message}</legend>
        <h1 className="text-3xl text-slate-700 mb-1">Login</h1>
        <div></div>
        <div>
          <input
            type="email"
            className="h-12 w-full border border-solid border-slate-300 rounded  pl-2"
            placeholder="Email"
            required
            name="email"
            value={email}
        ...
        <div>
          <button
            type="submit"
            className="brand text-white h-12 w-full border border-solid border-slate-300 rounded pl-2"
          >
            Login
          </button>
        </div>
        <div className="button flex justify-center gap-3 items-center h-12 w-full border border-solid border-slate-300 rounded pl-2">
          <box-icon type="solid" name="envelope" color="#EF4444"></box-icon>
        ...
        <div>
          <h3>
            New to Zomato ?{" "}
            <a href="/register" className="font">
              Create Account
            </a>
          </h3>
        </div>
